(function () {
    ...
    function cloneNodeDeepWithNewIds(node, idMap) {
      ...
      const clone = cloneMapValue(node) || {};

      clone.id = newId;
      clone.captions = (node.captions || []).map((caption) => ({
        ...cloneMapValue(caption),
        id: uid(),
      }));
      clone.children = (node.children || []).map((child) => {
        return cloneNodeDeepWithNewIds(child, idMap);
      });
    ...
    function collectSideMapsForIds(ids) {
      ...
      for (const oldId of ids) {
        ...
        if (window.__markMap?.[oldId]) {
          markMap[oldId] = cloneMapValue(window.__markMap[oldId]);
        }
      }

      return { fmtMap, colorFmtMap, blockBgMap, markMap };
    }
    ...
    function copySideMapsFromStoredItem(item, newIdMap) {
      ...
      for (const [originalId, clipboardId] of originalToClipboardId.entries()) {
        ...
        if (item.markMap?.[originalId]) {
          window.__markMap[newId] = cloneMapValue(item.markMap[originalId]);
        }
      }
    }
    ...
    function removeSourceIds(ids) {
      ...
      for (const id of topmost) {
        ...
        const removedIds = typeof collectSubtreeIds === "function"
          ? collectSubtreeIds(found.node)
          : [id];
  
        found.parent.children = (found.parent.children || []).filter((x) => x.id !== id);

        if (typeof clearNodeMetadata === "function") {
