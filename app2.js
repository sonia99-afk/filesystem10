...
function updateSchemaSelectionWithoutRender() {
  ...
  /*
    Переносим общий серый блок
    на новый активный объект.
  */
...
function renderSchemaView() {
  ...
  requestAnimationFrame(
    () => {
      ...
      /*
        Повторяем после полного
        расчёта размеров текста.
      */
...
/* =========================================================
   Текстовые свойства в Структуре
========================================================= */

function getSchemaTextInterfaceSettings() {
  const ui =
    window.viewSettings
      ?.getActiveSettings?.()
      ?.interface || {};
  ...
  return {
    showHeaders:
      ui.showTextPropertyHeaders !== false,

    headerPlacement:
      ui.textPropertyHeaderPlacement ===
      "above"
        ? "above"
        : "inline",
  };
...
function plainTextFromSchemaHtml(
  html
) {
  if (!html) {
    return "";
...
function getSchemaTextPropertyValue(
  node,
  propertyId
) {
  const props =
  ...
  const plainText =
    text.trim() ||
    plainTextFromSchemaHtml(
      html
    );

  /*
    Пустое свойство полностью скрываем.
  */
...
function sortSchemaTextProperties(
  instances
) {
  const state =
    window.viewTabs
  ...
  return instances
    ...
    .sort(
      (
        ...
        /*
          Новые свойства, которых ещё нет
          в сохранённом порядке, остаются
          в порядке создания.
        */
...
function renderSchemaTextProperties(
  node,
  li
) {
  const instances =
    sortSchemaTextProperties(
  ...
  const interfaceSettings =
    getSchemaTextInterfaceSettings();
  ...
  instances.forEach(
    (instance) => {
      ...
      property.dataset.nodeId =
        node.id;

      property.dataset.propertyId =
        instance.id;
  ...
  /*
    Если у объекта все текстовые
    свойства пустые — контейнер
    не добавляем.
  */
...
row.appendChild(label);
  ...
  {
  window.schemaTextPropertyEditor
    ?.appendButton?.(
      act,
      n
    );
  ...
  row.addEventListener('keydown', (e) => {
    ...
    if (isHotkey(e, "moveDown")) {
      ...
      /*
        Для начала переименования
        полный render не нужен.

        Строка уже существует в DOM,
        поэтому сразу открываем редактор.
      */
  ...
  */
  ...
  if (
  !options.suppressTextProperties
) {
  renderSchemaTextProperties(
    n,
    li
  );
}
...
function layoutCollapseColumn() {
  ...
  /*
  В объединённом режиме кнопки должны находиться
  внутри закреплённой панели структуры.
*/
...
const treeBox =
  controlsHost.getBoundingClientRect();
...
const rows =
  Array.from(
    ...
    (row) => {
      ...
      /*
        Полностью скрытая строка
        не участвует в раскладке кнопок.
      */
      ...
      /*
        ...
        строка должна реально находиться
        в видимой раскладке браузера.
      */
      ...
      /*
        Сворачивание требуется только
        объектам с дочерними объектами.
      */
...
window.visibilityControls?.apply(
  col,
  {
    ...
    /*
      Кнопка разворачивания должна быть видна,
      даже когда курсор находится вне строки.
    */
    alwaysVisible:
      isCollapsed,
  }
...
function layoutLevelCollapseBar() {
  ...
  /*
  В объединённом режиме верхние кнопки
  размещаем внутри закреплённой структуры.
*/
...
/*
  Сворачивание скрытого уровня
  должно становиться неактивным.
*/
...
window.addEventListener(
  ...
  if (isHotkey(e, "moveDown")) {
    ...
    /*
      Строка уже находится в DOM.
      Полная перерисовка перед
      переименованием не требуется.
    */
