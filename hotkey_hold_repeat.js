...
/*
  Действия горизонтальной иерархии
  и горизонтального айсикла.
*/
const HORIZONTAL_REPEAT_ACTIONS = [
  "horizontalListUp",
  "horizontalListDown",
  ...
  /*
    Остальные действия общие
    для структурных отображений.
  */
  "rangeUp",
  "rangeDown",
...
function runHorizontalAction(action) {
  if (!canRunTreeActionNow()) {
    stop();
    return false;
  }
  ...
  switch (action) {
    ...
    /*
      Мультивыбор и перемещение остаются
      общими для списков и горизонтальных видов.

      Их выполнение уже реализовано
      в runTreeAction().
    */
    default:
...
function runAction(action) {
  ...
  if (group === "horizontal") {
    return runHorizontalAction(action);
  }
