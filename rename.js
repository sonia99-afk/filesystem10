...
function placeEditingCaretAtEnd(
  editor
) {
  const selection =
    window.getSelection();
...
function startRename(id) {
  ...
  const row =
    tree?.querySelector(
      `.row[data-id="${cssEscape(id)}"]`
    );

  /*
    Если строка ещё не создана, сохраняем
    запрос до ближайшего render().

    Если строка уже существует, повторный
    отложенный запрос не нужен.
  */
  ...
  /*
    Убираем старый ожидающий запрос.

    Иначе следующий requestAnimationFrame
    повторно вызовет startRename(), удалит
    уже созданный редактор и вызовет blur.
  */

  renamingId = null;

/*
  В обычных видах label находится
  прямо в строке, а в Айсиклах —
  внутри main-контейнера.
*/
...
const labelHost =
  ...
  row.querySelector(
    ":scope > .icicle-horizontal-main > .label"
  ) ||
  row.querySelector(
    ":scope > .icicle-vertical-main > .label"
  );

if (!labelHost) return;

/*
  Сам label остаётся на месте
  и выполняет роль ячейки.
*/

labelHost.classList.add(
  "inline-edit-cell"
);
...
ed.className =
  "edit edit-rich inline-cell-editor";
...
labelHost.appendChild(ed);

  ed.focus({ preventScroll: true });

placeEditingCaretAtEnd(ed);
...
/*
  Описание остаётся внешней ячейкой,
  редактор появляется внутри.
*/
...
ed.className =
  "edit edit-rich edit-caption inline-cell-editor";
