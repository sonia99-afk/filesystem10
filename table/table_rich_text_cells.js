...
(function () {
  ...
  function startTableRichTextCellEditor(
  td,
  options
) {
  if (!td) return false;
  ...
  /*
    Запоминаем размеры исходной ячейки.
    Редактор не участвует в разметке
    таблицы и открывается поверх неё.
  */

  const initialCellRect =
  ...
  const minimumHeight =
    Math.max(
      28,
      Math.ceil(
        initialCellRect.height
      )
    );

  td.classList.add(
    "is-editing",
    "table-rich-cell-editing"
  );

  /*
    Содержимое td не удаляем.
    Благодаря этому высота строки
    таблицы вообще не изменяется.
  */
  ...
  editor.className =
    (
      ...
      "table-rich-cell-editor-multiline " +
      "table-rich-cell-overlay-editor"
    );
  ...
  */
  ...
  /*
    Положение редактора относительно
    исходной ячейки.
  */
  ...
  /*
    Редактор увеличивается по содержимому,
    но не выходит за границы экрана.
  */
  ...
  function resizeEditor() {
    ...
    /*
      Сначала возвращаем минимальные
      размеры, чтобы редактор мог
      не только увеличиваться,
      но и уменьшаться.
    */
  ...
  /*
    ...
    поэтому он не растягивает таблицу.
  */
  ...
  requestAnimationFrame(
    () => {
      ...
      /*
        Как на скриншоте и в прототипе:
        при открытии выделяется весь текст.
      */
...
/*
  Второй Text имеет key вроде:

  text__abc123

  но логический тип остаётся text.
*/
